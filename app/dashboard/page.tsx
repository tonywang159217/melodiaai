  ...
  return (
    <div className="mx-auto w-full max-w-6xl px-6 py-10 space-y-10">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <Link
          href="/pricing"
          className="inline-flex items-center justify-center rounded-full border border-border bg-background px-4 py-2 text-sm font-medium hover:bg-accent hover:text-accent-foreground"
        >
          Upgrade plan
        </Link>
      </div>
      ...
      <div className="rounded-2xl border border-border/60 bg-card/50 p-6">
        <div className="mb-3 flex items-center justify-between">
          ...
          <span className="text-sm text-muted-foreground">
            {credits} / {monthlyLimit} generations
          </span>
        </div>
        <div className="h-2 w-full overflow-hidden rounded-full bg-muted">
          <div
            className="h-full bg-gradient-to-r from-primary to-emerald-400 transition-all"
            style={{ width: `${Math.min(100, (credits / monthlyLimit) * 100)}%` }}
          />
        </div>
        <p className="mt-2 text-xs text-muted-foreground">
          {planId === "free" ? "Free plan" : planId === "basic" ? "Basic plan" : "Pro plan"}
        </p>
      </div>
    </div>
  );
